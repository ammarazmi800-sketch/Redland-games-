...
client.on('messageCreate', async (message) => {
    ...
    if (command === 'help' || command === 'اوامر' || command === 'مساعدة') {
        const helpEmbed = new EmbedBuilder()
            ...
            .setDescription('جميع الألعاب تدعم حتى **100+ لاعب** مع نظام انضمام زمني دقيق (60 ثانية) مثل سيرفر Hellwood الأصلي!')
            .addFields(
                { name: '🛡️ **أوامر الإدارة والحماية**', value: '`+نفي @User` - حظر العضو المخرب\n`+ايقاف` - إيقاف/تشغيل البوت', inline: false },
                { name: '🎮 **الألعاب الجماعية الكبرى (100 لاعب)**', value: '`+سالفة` - لعبة برا السالفة الشهيرة\n`+كراسي` - الكراسي الموسيقية (أزرار تفاعلية)\n`+غميضة` - غميضة (اختيار مكان + بحث العجلة)\n`+مافيا` - لعبة المافيا السرية (أدوار بالخاص + تصويت)\n`+قنبلة` - تفكيك القنبلة الموقوتة\n`+روليت` - عجلة الحظ العشوائية\n`+اسرع` - تحدي السرعة في الكتابة\n`+مبارزة @User` - معركة فردية', inline: false }
            )
            .setFooter({ text: 'Hellwood Community — جميع الحقوق محفوظة' });
    ...
    // ==========================================
    // 🤫 لعبة برا السالفة (تستوعب حتى 100 لاعب)
    // ==========================================
    if (command === 'سالفة' || command === 'برا_السالفة') {
        const embed = new EmbedBuilder()
            .setColor('#9B59B6')
            .setTitle('🤫 **لعبة برا السالفة — فتح باب الانضمام!**')
            .setDescription('امامكم **60 ثانية (دقيقة)** للانضمام للعبة عبر الضغط على الزر أدناه! (تستوعب حتى 100+ لاعب)');
    ...
    // ==========================================
    // 🪑 لعبة الكراسي الموسيقية (تستوعب 100 لاعب بدقيقة)
    // ==========================================
    if (command === 'كراسي') {
        const embed = new EmbedBuilder()
            .setColor('#F1C40F')
            .setTitle('🪑 **لعبة الكراسي الموسيقية الكبرى!**')
            .setDescription('امامكم **60 ثانية (دقيقة)** لحجز مقعدكم قبل انطلاق الموسيقى واستبعاد الخاسرين!');
    ...
    // ==========================================
    // 🙈 لعبة الغميضة (دقيقتين للانضمام والاختباء)
    // ==========================================
    ...
    // ==========================================
    // 🕵️‍♂️ لعبة المافيا السرية (تستوعب حتى 100 لاعب)
    // ==========================================
    if (command === 'مافيا') {
        const embed = new EmbedBuilder()
            .setColor('#2C3E50')
